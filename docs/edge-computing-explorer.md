...
- `_data/edge_catalog/paper.json`: Structured records of Tables I–V from arXiv:2505.09995v1. Forty-one current inventory resources map to all five tables. Merged paper rows are explicitly mapped to the separate catalog entries. SimEdgeIntel and Phantom occur in the paper but are not separate entries in the current README and are not silently added or equated with similarly named projects.
...
- Search covers names, aliases, English and Chinese summaries, and facet values. Results are alphabetical within documented and tentative groups. Category bars show matching counts with the category constraint removed; sidebar counts show total catalog coverage.
